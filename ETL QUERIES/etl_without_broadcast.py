...
spark = SparkSession.builder.appName('Amazon Product Categories ETL').getOrCreate()
...
def main(inputs,output,start_year,end_year):
    ...
    #raw_dataset = raw_dataset.repartition(10)
    ...
    #Using Broadcast join since product_count dataframe is very very small compared to verified_purchases_df in the below join operation
    #ten_core_dataset = verified_purchases_df.join(broadcast(product_count.select('product_id')),on='product_id')
    
    #Below is a normal(Shuffle) Join
    ten_core_dataset = verified_purchases_df.join(product_count.select('product_id'),on='product_id')
