...
def main(inputs,output,start_year,end_year):
    ...
    sliced_data = spark.sql("SELECT customer_id,review_id,product_id,product_title,\
    	star_rating,helpful_votes,total_votes,review_headline,\
    	review_body,review_date from ten_core_dataset \
    	WHERE year(review_date) BETWEEN "+start_year+" AND "+end_year)
    sliced_data.write.option("sep","\t").option("header","true").csv(output+"_"+start_year+"_"+end_year)
