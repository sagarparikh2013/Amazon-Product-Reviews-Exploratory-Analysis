...
def main(inputs):
    ...
    helpful_df = helpful_df.groupBy('product_category').agg(functions.mean(helpful_df.helpfulness_percentage).alias('mean_helpfulness_percent')).cache()
    helpful_df.repartition(1).write.mode('overwrite').csv('category_mean_helpfulness')
